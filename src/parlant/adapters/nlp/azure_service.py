...
class AzureEstimatingTokenizer(EstimatingTokenizer):
    def __init__(self, model_name: str) -> None:
        ...
        self.encoding_model_name = os.environ["AZURE_EMBEDDING_DEPLOYMENT_ID"]
        self.encoding = tiktoken.encoding_for_model(self.encoding_model_name)
...
class GPT_4o(AzureSchematicGenerator[T]):
    def __init__(self, logger: Logger) -> None:
        # read the deployment names from environment variables
        model_name = os.environ["OPENAI_DEPLOYMENT_ID"]
        _client = AsyncAzureOpenAI(
            api_key=os.environ["AZURE_API_KEY"],
            azure_endpoint=os.environ["AZURE_ENDPOINT"],
            api_version="2024-08-01-preview",
        )
        super().__init__(model_name=model_name, logger=logger, client=_client)
